...
test.describe('Dashboard', () => {
  ...
  for (const [status, label] of [['completed', 'Completed'], ['cancelled', 'Cancelled']] as const) {
  ...
  test('next appointment is the earliest future active or pending record',
    ...
    async ({ dashboardPage, db, testUser }) => {
      ...
      await expect(dashboardPage.nextAppointmentDoctor).toHaveText(`Dr. ${doctor.first_name} ${doctor.last_name}`);
      ...
      await expect(dashboardPage.nextAppointment).toContainText(next.status === 'active' ? 'Confirmed' : 'Pending');
    });
