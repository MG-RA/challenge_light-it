...
test('dashboard upcoming counter changes after a persisted booking and deletion', { tag: '@mutating' }, async ({ page, dashboardPage, owned, db, testUser }) => {
  ...
  await page.reload();
  await expect.soft(counter, 'counter increases after persisted booking and reload').toHaveText(String(before + 1));
  ...
  await page.reload();
  await expect.soft(counter, 'counter returns to baseline after deletion and reload').toHaveText(String(before));
  test.info().annotations.push({ type: 'observation', description: `Verified DB upcoming counts: ${before} -> ${before + 1} -> ${before}` });
