...
test(
  'login throttles a bounded series of failed attempts',
  { tag: '@rate-limit' },
  async ({ anonApi, credentials }) => {
    test.skip(
      process.env.RUN_RATE_LIMIT !== '1',
      'Explicit bounded security run, isolated from normal login tests',
    );
    ...
    for (let attempt = 0; attempt < 10; attempt++) {
      const response = await anonApi.login({
    ...
    test.info().annotations.push({
      type: 'observation',
      description: `Failed attempt statuses: ${statuses.join(', ')}`,
    });
    expect(
      statuses,
      'proposed policy: throttle within 10 failed attempts; no 429 is not proof of unlimited attempts',
    ).toContain(429);
  },
);
