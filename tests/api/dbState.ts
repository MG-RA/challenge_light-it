...
function matches(row: object | undefined, expected: object): boolean {
  return row !== undefined && Object.entries(expected).every(([key, value]) =>
    (row as Record<string, unknown>)[key] === value);
}

/** The owner's appointment row settled on `expected`. */
export async function expectStoredAppointment(
  db: Db, id: number, ownerId: number, expected: Partial<StateAppointmentRow>,
): Promise<StateAppointmentRow> {
  const row = await settle(() => db.stateAppointment(id, ownerId), (r) => matches(r, expected));
...
export async function expectAppointmentUnchanged(
  db: Db, ownerId: number, before: StateAppointmentRow,
): Promise<void> {
  const row = await settle(() => db.stateAppointment(before.id, ownerId), (r) => matches(r, before));
...
export async function expectAppointmentGone(db: Db, id: number, ownerId: number): Promise<void> {
  const row = await settle(() => db.stateAppointment(id, ownerId), (r) => r === undefined);
...
export async function expectNothingStored(db: Db, marker: string, ownerId: number): Promise<void> {
  const rows = await settle(() => db.markedAppointments(marker, ownerId), (r) => r.length === 0);
...
export async function expectStoredPayments(
  db: Db, appointmentId: number, expected: Partial<PaymentRow>[],
): Promise<PaymentRow[]> {
  const rows = await settle(() => db.paymentsForAppointment(appointmentId),
...
export async function expectStoredNotifications(
  db: Db, ownerId: number, expected: NotificationRow[],
): Promise<void> {
  const rows = await settle(() => db.notificationsForUser(ownerId),
