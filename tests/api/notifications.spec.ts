...
test('GET /notifications contains only the user records and maps isRead to is_read', async ({ api, db, testUser }) => {
  const notifications = await expectCompleteJson(await api.listNotifications(), 200, 'Notification[]');
  ...
  for (const row of rows) {
    ...
    expect(notifications.find((n) => n.id === row.id)).toMatchObject({ ...stored, isRead: is_read });
  }
...
test('PUT /notifications/:id/read changes only that read flag and repeats idempotently', { tag: '@mutating' },
  async ({ owned, api, db, testUser }) => {
    ...
    const target = current.find((n) => !n.is_read && !baseline.some((b) => b.id === n.id)
