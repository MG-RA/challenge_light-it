...
test.describe('Authorization boundaries', () => {
  test("appointment detail refuses another patient's appointment", async ({ api, db, testUser }) => {
    ...
    expect(body ?? {},'a refusal must not disclose appointment data').not.toHaveProperty('patient_id');
  });

  test('profile rejects a token whose user_id was altered', async ({ anonApi, db, testUser, token }) => {
    const other = await db.otherPatientAppointment(testUser.id);
    ...
    expect(decodeJwtPayload(token), 'session token identity claim').toMatchObject({ user_id: testUser.id });
  ...
  test('profile answers an empty-signature token with the documented 401',
    { annotation: { type: 'issue', description: 'F-20: edge firewall returns text/plain 403 (docs/FINDINGS.md)' } },
    async ({ anonApi, token }) => {
