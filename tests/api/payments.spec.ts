...
test('GET /payments matches payments for the patient appointments', async ({
  api,
  db,
  testUser,
}) => {
...
// Payments cannot be removed through the API. Each test pays one dedicated owned appointment, the two
// tests together use the four capped submissions, and teardown declares any residue.
test(
  'POST /payments stores one valid payment and rejects a duplicate',
  { tag: '@mutating' },
  async ({ owned, db }) => {
    ...
    // A duplicate is only meaningful once a valid payment exists, so it stays in this test.
    const duplicate = await owned.pay(booked.id, { amount: fee, method: 'cash' });
    expect
      .soft([400, 409], 'duplicate payment rejection is a proposed business expectation')
      .toContain(duplicate.status);
    await expectStoredPayments(db, booked.id, [payment!]);
  },
);

// Independent of a successful payment, so F-18 cannot hide these checks.
test(
  'POST /payments rejects zero and negative amounts without storing a payment',
  { tag: '@mutating' },
  async ({ owned, db }) => {
    const booked = await owned.book();
    for (const [label, amount] of [
      ...
      await test.step(`rejects a ${label} amount`, async () => {
        ...
        expect
          .soft([400, 409], `${label} payment rejection is a proposed business expectation`)
          .toContain(result.status);
        await expectStoredPayments(db, booked.id, []);
      });
    }
  },
);
