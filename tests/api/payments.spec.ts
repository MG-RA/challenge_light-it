...
test('POST /payments stores one valid payment and rejects a duplicate', { tag: '@mutating' },
  async ({ owned, db }) => {
    ...
    expect.soft([400, 409], 'duplicate payment rejection is a proposed business expectation').toContain(duplicate.status);
    await expectStoredPayments(db, booked.id, [payment!]);
  });

// Independent of a successful payment, so F-18 cannot hide these checks.
test('POST /payments rejects zero and negative amounts without storing a payment', { tag: '@mutating' },
  async ({ owned, db }) => {
    const booked = await owned.book();
    for (const [label, amount] of [['zero', 0], ['negative', -1]] as const) {
