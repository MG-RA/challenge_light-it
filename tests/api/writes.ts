...
import type { CreateAppointmentRequest, CreatePaymentRequest, RescheduleAppointmentRequest } from '../../src/api/types';
...
export class OwnedData {
  ...
  async freeSlot(doctor?: DoctorRow, exclude?: { appointment_date: string; time_slot: string }): Promise<Slot> {
    ...
    for (let offset = 0; offset < 30; offset++) {
      ...
      for (const time of slots as string[]) {
        ...
        if (!occupied.some((a) => a.appointment_date === date && a.time_slot === time)) {
          return { doctor: selected, body: { doctor_id: selected.id, appointment_date: date, time_slot: time } };
        }
      }
    }
  ...
  async create(body: Partial<CreateAppointmentRequest>) {
    ...
    return { status: response.status(), body: await response.json().catch(() => undefined), marker, payload, rows };
  }
  ...
  async book(doctor?: DoctorRow) {
    ...
    const occupied = await this.db.occupiedSlots(slot.doctor.id, slot.body.appointment_date, slot.body.appointment_date);
    expect(occupied.some((a) => a.time_slot === slot.body.time_slot), 'slot still free before creation').toBe(false);
  ...
  private async ownedRow(id: number): Promise<StateAppointmentRow> {
    ...
    if (!row || row.notes !== marker) {
      throw new Error(`Refusing to mutate appointment ${id}: not verified as created and owned by this run`);
    }
    return row;
  }
  ...
  async reschedule(id: number, body: Partial<RescheduleAppointmentRequest>) {
    ...
    return { status: response.status(), body: await response.json().catch(() => undefined), before };
  }
  ...
  async pay(id: number, body: Omit<CreatePaymentRequest, 'appointment_id'>) {
    ...
    if (++paymentSubmissions > PAYMENT_CAP) throw new Error(`Payment submission cap (${PAYMENT_CAP}) reached`);
  ...
  async cleanup(): Promise<string[]> {
    ...
    for (const [id, marker] of this.created) {
      try {
        ...
        const response = await this.api.deleteAppointment(id);
        const after = await observe(() => this.db.stateAppointment(id, this.ownerId), (r) => r === undefined)
...
export const test = base.extend<{ owned: OwnedData }>({
  owned: async ({ api, db, testUser }, use, info) => {
    if (!mutationsEnabled(process.env.RUN_MUTATING)) throw new Error('Remote writes require RUN_MUTATING=1');
    ...
    expect(await identity.json(), 'API token must belong to the configured DB account').toMatchObject({ id: testUser.id });

    const owned = new OwnedData(api, db, testUser.id);
    try {
      await use(owned);
    } finally {
      for (const note of await owned.cleanup()) info.annotations.push({ type: 'residue', description: note });
    }
  },
});
