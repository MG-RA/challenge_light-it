...
export class DashboardPage {
  ...
  constructor(private readonly page: Page) {
    ...
    this.nextAppointment = this.main
      .locator('section')
      .filter({ has: page.getByRole('heading', { name: 'Your next appointment' }) })
      .describe('Next appointment panel');
    this.nextAppointmentDoctor = this.nextAppointment
      .getByTestId('next-appointment-doctor')
      .describe('Next appointment doctor');
    this.viewAllAppointments = this.main
      .getByRole('link', { name: 'View all', exact: true })
      .describe('"View all" appointments link');
  }
  ...
  counter(label: DashboardCounter): Locator {
    if (label === 'Upcoming appointments')
      return this.main.getByTestId('upcoming-count').describe('Upcoming appointments count');
